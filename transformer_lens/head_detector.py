...
HEAD_NAMES = ["previous_token_head", "duplicate_token_head", "induction_head", "translation_head"]
...
def detect_head(
    ...
    assert (head_name is None) != (
        detection_pattern is None
    ), "Exactly one of head_name or detection_pattern must be specified."
    device = model.cfg.device

    if head_name is not None:
        assert head_name in HEAD_NAMES, "Head name not valid."
        detection_pattern = eval(
            f"get_{head_name}_detection_pattern(model, sequence)"
        ).to(device)

    sequence = model.to_tokens(sequence).to(device)

    assert (
        1 < sequence.shape[-1] < model.cfg.n_ctx
    ), "The sequence must be non-empty and must fit within the model's context window."
    assert (
        sequence.shape[-1] == detection_pattern.shape[0] == detection_pattern.shape[1]
    ...
    for head in heads:
        if head not in specific_heads:
            matches.append(
                -1
            )  # This is kinda ugly, the idea is to be able to still plot this on a 2D grid and everything not in the circuit is red. Ideas?
        else:
            ...
            matches.append(
                (torch.sum(matched_attention) / torch.sum(attention_pattern))
            )  # Return total percentage of attention which matches the detection pattern.
...
# Translation Heads
def get_translation_head_detection_pattern(
    model: HookedTransformer,
    sequence: str,
) -> torch.Tensor:
    """Outputs a detection score for translation heads.These are attention heads that attend
    to the the token after an earlier copy of the current token in another language. Read more about them here
    https://dynalist.io/d/n2ZWtnoYHrU1s4vnFSAQ519J#z=to1dknSULFmpfYEfJZkV1M78

    Args:
      model: Model being used.
      sequence: String being fed to the model.
    Returns:
      torch.Tensor: A detection pattern for translation heads.
    """
